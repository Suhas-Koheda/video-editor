...
def render_with_screenshots(video_path, render_plan):
    ...
    # We build a complex filter string for FFmpeg
    # [0:v][1:v] overlay=x=40:y=40:enable='between(t,1,5)' [v1];
    # [v1][2:v] overlay=x=40:y=40:enable='between(t,10,15)' [v2] ...
    ...
    for i, seg in enumerate(render_plan):
        ...
        # Scale the screenshot to a reasonable size (e.g., width 400)
        # and place it on the screen
        label = f"[v{i+1}]"
        screenshot_idx = i + 1
        
        # Filter: scale screenshot, then overlay it during the 'between' time
    ...
    if result.returncode != 0:
        # Fallback for very long filter strings if needed, 
        # but for common projects this should work.
